...
// The following options can be used with web.Server.HandleBrowser and related methods

// SecFetchOpt is a HandleOpt for web.Server methods that enables secfetch protection.
// It allows passing secfetch-specific options through the web.HandleOpt interface.
//
// Example:
//
//	server.HandleBrowser("/path", handler, secfetch.SecFetchOpt{
//		Options: []secfetch.Option{
//			secfetch.WithAllowedModes("navigate", "cors"),
//		},
//	})
type SecFetchOpt struct {
	Options []Option
}

// AllowNav is a HandleOpt that allows cross-site navigation.
// It's equivalent to AllowCrossSiteNavigation but designed for use with web.HandleOpt.
//
// Example:
//
//	server.HandleBrowser("/public", handler, secfetch.WebProtect, secfetch.AllowNav{})
type AllowNav struct{}

// AllowAPI is a HandleOpt that allows cross-site API requests.
// It's equivalent to AllowCrossSiteAPI but designed for use with web.HandleOpt.
//
// Example:
//
//	server.HandleBrowser("/api", handler, secfetch.WebProtect, secfetch.AllowAPI{})
type AllowAPI struct{}

// WebProtect is middleware that integrates with web.Server's HandleOpt system.
// This allows using secfetch protection with web.Server.HandleBrowser, etc.
//
// Example:
//
//	server.HandleBrowser("/path", handler, secfetch.WebProtect)
//	server.HandleBrowser("/api", apiHandler, secfetch.WebProtect, secfetch.AllowAPI{})
func WebProtect(h http.Handler, webOpts ...interface{}) http.Handler {
	var fetchOpts []Option

	// Convert web.HandleOpt to secfetch.Option
	for _, opt := range webOpts {
		switch o := opt.(type) {
		case SecFetchOpt:
			fetchOpts = append(fetchOpts, o.Options...)
		case AllowNav:
			fetchOpts = append(fetchOpts, AllowCrossSiteNavigation{})
		case AllowAPI:
			fetchOpts = append(fetchOpts, AllowCrossSiteAPI{})
		}
	}

	return Protect(h, fetchOpts...)
}
